...
class VideoEditorUI:
    ...
    def _load_icons(self):
        ...
        for name in icon_names:
            path = "" # Initialize path to avoid reference before assignment warning
            try:
                path = os.path.join("assets", f"{name}_icon.png")
                # FIXED: Use Image.Resampling.LANCZOS for newer Pillow versions
    ...
    def _show_trim_panel(self): self._show_panel("Trim Tool", self.trim_frame)
    def _show_adjust_panel(self): self._show_panel("Light", self.adjust_frame)
    def _show_color_panel(self): self._show_panel("Colour", self.color_frame)
    def _show_filter_panel(self): self._show_panel("Filters", self.filter_frame)
    ...
    def _create_panel_with_sliders(self, parent, items):
        ...
        for name, props in items.items():
            ...
            self.sliders[key] = scale # Store slider for resetting
        return frame
    ...
    def _create_filter_panel(self, parent):
        frame = ttk.Frame(parent)
        ttk.Button(frame, text="Grayscale", command=lambda: self._apply_filter('grayscale')).pack(pady=10, fill='x', padx=10)
        ttk.Button(frame, text="Flip Horizontally", command=lambda: self._apply_filter('mirror_x')).pack(pady=10, fill='x', padx=10)
        return frame
    ...
    def _draw_timeline(self):
        self.timeline_canvas.delete("all"); self.track_header_canvas.delete("all")
        track_height, ruler_height, v1_y, audio1_y = 60, 25, 25, 85
        self.track_header_canvas.create_text(40, v1_y + track_height / 2, text="V1", fill=self.TEXT_COLOR, font=('Segoe UI', 12, 'bold'))
        self.track_header_canvas.create_text(40, audio1_y + track_height / 2, text="Audio 1", fill=self.TEXT_COLOR, font=('Segoe UI', 10))
        total_duration = max(self.processor.clip.duration if self.processor.clip else 0, self.processor.audio_clip.duration if self.processor.audio_clip else 0, 60)
        if total_duration < 30: step = 2
        elif total_duration < 120: step = 10
        elif total_duration < 600: step = 30
        else: step = 60
        def format_time(seconds): mins, secs = divmod(int(seconds), 60); return f"{mins:02d}:{secs:02d}"
        ...
        if self.processor.clip: self.timeline_canvas.create_rectangle(0, v1_y, self.processor.clip.duration * self.pixels_per_second, v1_y + track_height, fill=self.ACCENT_COLOR_VIDEO, outline="#000")
        if self.processor.audio_clip: self.timeline_canvas.create_rectangle(0, audio1_y, self.processor.audio_clip.duration * self.pixels_per_second, audio1_y + track_height, fill=self.ACCENT_COLOR_AUDIO, outline="#000")
        self.timeline_canvas.config(scrollregion=(0, 0, total_width, 250)); self._draw_playhead()
    ...
    def _resize_preview(self, _event=None):
        # FIXED: Renamed 'event' to '_event' to indicate it's not used
    ...
    def _reset_sliders(self):
        ...
        for key, slider in self.sliders.items():
            if key in ["r", "g", "b", "gamma", "speed", "volume"]:
                slider.set(1.0)
            else:
                slider.set(0.0)
